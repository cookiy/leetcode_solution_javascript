/**给定两个二叉树，想象当你将它们中的一个覆盖到另一个上时，两个二叉树的一些节点便会重叠。

你需要将他们合并为一个新的二叉树。合并的规则是如果两个节点重叠，那么将他们的值相加作为节点合并后的新值，否则不为 NULL 的节点将直接作为新二叉树的节点。

示例 1:

输入: 
	Tree 1                     Tree 2                  
          1                         2                             
         / \                       / \                            
        3   2                     1   3                        
       /                           \   \                      
      5                             4   7                  
输出: 
合并后的树:
	     3
	    / \
	   4   5
	  / \   \ 
	 5   4   7
注意: 合并必须从两个树的根节点开始。

来源：力扣（LeetCode）
链接：https://leetcode-cn.com/problems/merge-two-binary-trees
著作权归领扣网络所有。商业转载请联系官方授权，非商业转载请注明出处。 */
/**
 * Definition for a binary tree node.
 * function TreeNode(val) {
 *     this.val = val;
 *     this.left = this.right = null;
 * }
 */
/**
 * @param {TreeNode} t1
 * @param {TreeNode} t2
 * @return {TreeNode}
 */
var mergeTrees = function(t1, t2) {
    console.log('input t1', t1);
    console.log('input t2', t2);
    if (t1 === null) {
        return t2;
    } else if (t2 === null) {
        return t1;
    } else {
        // 层级节点
        t1.val = t1.val + t2.val
        t1.left = mergeTrees(t1.left, t2.left);
        t1.right = mergeTrees(t1.right, t2.right);
        console.log('output', t1)
        return t1
    }
};

/**
input t1 TreeNode {
  val: 1,
  right: TreeNode { val: 2, right: null, left: null },
  left:
   TreeNode {
     val: 3,
     right: null,
     left: TreeNode { val: 5, right: null, left: null } } }
input t2 TreeNode {
  val: 2,
  right:
   TreeNode {
     val: 3,
     right: TreeNode { val: 7, right: null, left: null },
     left: null },
  left:
   TreeNode {
     val: 1,
     right: TreeNode { val: 4, right: null, left: null },
     left: null } }
input t1 TreeNode {
  val: 3,
  right: null,
  left: TreeNode { val: 5, right: null, left: null } }
input t2 TreeNode {
  val: 1,
  right: TreeNode { val: 4, right: null, left: null },
  left: null }
input t1 TreeNode { val: 5, right: null, left: null }
input t2 null
input t1 null
input t2 TreeNode { val: 4, right: null, left: null }
output TreeNode {
  val: 4,
  right: TreeNode { val: 4, right: null, left: null },
  left: TreeNode { val: 5, right: null, left: null } }
input t1 TreeNode { val: 2, right: null, left: null }
input t2 TreeNode {
  val: 3,
  right: TreeNode { val: 7, right: null, left: null },
  left: null }
input t1 null
input t2 null
input t1 null
input t2 TreeNode { val: 7, right: null, left: null }
output TreeNode {
  val: 5,
  right: TreeNode { val: 7, right: null, left: null },
  left: null }
output TreeNode {
  val: 3,
  right:
   TreeNode {
     val: 5,
     right: TreeNode { val: 7, right: null, left: null },
     left: null },
  left:
   TreeNode {
     val: 4,
     right: TreeNode { val: 4, right: null, left: null },
     left: TreeNode { val: 5, right: null, left: null } } } 
*/
